...
fn history_add_and_undo() {
    ...
    let poped_value_1 = history.undo_last_move().unwrap();
...
fn history_pop_if_empty() {
    let mut history = GameHistory::new();
    let poped_value_1: Option<Move> = history.undo_last_move();
...
fn history_check_if_undo_deletes() {
    ...
    let poped_value_1 = history.undo_last_move().unwrap();
    assert_eq!(pushed_value_1, poped_value_1);
    let poped_value_2 = history.undo_last_move();
...
fn history_multiple_adds_and_undos() {
    ...
    let poped_value_2 = history.undo_last_move().unwrap();
    assert_eq!(pushed_value_1, poped_value_2);
    let poped_value_3 = history.undo_last_move();
